...
public class LimitValve : KMonoBehaviour, ISaveLoadable
{
  ...
  public float Amount
  {
    ...
    {
      this.m_amount = value;
      this.BoxingTrigger<float>(-1722241721, this.m_amount);
      this.Refresh();
    }
  }
  ...
  protected override void OnPrefabInit()
  {
    ...
    {
      if (componentsInChild.name.Contains("_fg"))
      {
        this.fg_Controller = componentsInChild;
        break;
      }
    }
  }
    ...
    Pickupable pickupable)
  {
    ...
    if (this.lastElemenet != element && this.conduitType == ConduitType.Liquid)
    {
      ...
      {
        Color colour = (Color) elementByHash.substance.colour with
        {
          a = 1f
        };
        this.controller.SetSymbolTint(new KAnimHashedString("gradient"), colour);
        this.fg_Controller.SetSymbolTint(new KAnimHashedString("water_color_fg"), colour);
      }
    }

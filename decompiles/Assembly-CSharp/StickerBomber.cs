    ...
    GameStateMachine<StickerBomber, StickerBomber.Instance, IStateMachineTarget, object>.GameInstance(master)
  {
    ...
    private class StickerBombReactable : Reactable
    {
      ...
      private int FindPlacementCell()
      {
        ...
        GameUtil.FloodFillConditional((Queue<GameUtil.FloodFillInfo>) queue, this.canPlaceStickerCb, (HashSet<int>) visited_cells, (List<int>) pooledList, 2);
        visited_cells.Recycle();
        queue.Recycle();
        int random = pooledList.Count > 0 ? pooledList.GetRandom<int>() : 0;
        pooledList.Recycle();
        return random;
      }

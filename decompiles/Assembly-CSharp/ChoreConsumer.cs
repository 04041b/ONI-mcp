...
public class ChoreConsumer : KMonoBehaviour, IPersonalPriorityManager
{
  ...
  private static Func<object, ChoreConsumer, Util.IterationInstruction> FindNextChoreEvaluateEntryHelper = (Func<object, ChoreConsumer, Util.IterationInstruction>) ((obj, consumer) =>
  {
    if (!(obj is FetchChore fetchChore2))
    {
      DebugUtil.Assert(false, "FindNextChore found an entry that wasn't a FetchChore");
      return Util.IterationInstruction.Continue;
    }
    if (fetchChore2.target == null)
    {
      DebugUtil.Assert(false, "FindNextChore found an entry with a null target");
      return Util.IterationInstruction.Continue;
    }
    if (fetchChore2.isNull)
    {
      Debug.LogWarning((object) "FindNextChore found an entry that isNull");
      return Util.IterationInstruction.Continue;
    }
    int cell = Grid.PosToCell(fetchChore2.gameObject);
    if (consumer.consumerState.solidTransferArm.IsCellReachable(cell))
      fetchChore2.CollectChoresFromGlobalChoreProvider(consumer.consumerState, consumer.preconditionSnapshot.succeededContexts, consumer.preconditionSnapshot.failedContexts, false);
    return Util.IterationInstruction.Continue;
  });

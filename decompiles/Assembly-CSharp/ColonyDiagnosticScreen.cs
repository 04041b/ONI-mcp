...
public class ColonyDiagnosticScreen : KScreen, ISim1000ms
{
  ...
  protected override void OnSpawn()
  {
    ...
    this.seeAllButton.onClick += (System.Action) (() => AllDiagnosticsScreen.Instance.Show(!AllDiagnosticsScreen.Instance.IsScreenActive()));
  }
  ...
  private class DiagnosticRow : ISim4000ms
  {
    ...
    private IEnumerator VisualNotificationRoutine()
    {
      ...
      {
        contentRect.anchoredPosition = Vector2.left * (Mathf.Sin(i * 3.14159274f) * 50f);
      ...
      for (i = 0.0f; (double) i < (double) bounceDuration; i += Time.unscaledDeltaTime)
      {
        contentRect.anchoredPosition = Vector2.left * (Mathf.Sin(i * 3.14159274f) * 25f);
        indicator.sizeDelta = this.defaultIndicatorSizeDelta + Vector2.one * (float) Mathf.RoundToInt(Mathf.Sin((float) (6.0 * (3.1415927410125732 * ((double) i / (double) bounceDuration)))));
        yield return (object) 0;
      }
      contentRect.anchoredPosition = Vector2.zero;
      for (i = 0.0f; (double) i < (double) bounceDuration; i += Time.unscaledDeltaTime)
      {
        contentRect.anchoredPosition = Vector2.left * (Mathf.Sin(i * 3.14159274f) * 12f);
        indicator.sizeDelta = this.defaultIndicatorSizeDelta + Vector2.one * (float) Mathf.RoundToInt(Mathf.Sin((float) (6.0 * (3.1415927410125732 * ((double) i / (double) bounceDuration)))));
        yield return (object) 0;
      }
      contentRect.anchoredPosition = Vector2.zero;

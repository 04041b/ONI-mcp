...
public class ElectricalUtilityNetwork : UtilityNetwork
{
    ...
    List<WireUtilityNetworkLink>[] bridgeGroups)
  {
    ...
    if (flag)
    {
      ...
      if ((UnityEngine.Object) this.targetOverloadedWire != (UnityEngine.Object) null)
        this.targetOverloadedWire.BoxingTrigger<BuildingHP.DamageSourceInfo>(-794517298, new BuildingHP.DamageSourceInfo()
        {
          damage = 1,
  ...
  public float GetMaxSafeWattage(List<WireUtilityNetworkLink>[] bridgeGroups)
  {
    ...
    {
      List<Wire> wireGroup = this.wireGroups[rating];
      int num1 = wireGroup == null ? 0 : (wireGroup.Count > 0 ? 1 : 0);
      List<WireUtilityNetworkLink> bridgeGroup = bridgeGroups[rating];
      int num2 = bridgeGroup == null ? (false ? 1 : 0) : (bridgeGroup.Count > 0 ? 1 : 0);
      if ((num1 | num2) != 0)

  ...
  GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance>
{
    ...
    GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
    ...
    {
      ReportManager.Instance.ReportValueWithGameObjectContext(ReportManager.ReportType.DiseaseStatus, (float) this.primaryElement.DiseaseCount, this.master.gameObject, StringFormatter.Replace((string) UI.ENDOFDAYREPORT.NOTES.GERMS, "{0}", this.master.name));
    }
    ...
    private void OnBionicTagsChanged(object o)
    {
      if (o == null)
        return;
      TagChangedEventData changedEventData = ((Boxed<TagChangedEventData>) o).value;

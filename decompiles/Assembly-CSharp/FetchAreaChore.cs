...
public class FetchAreaChore : Chore<FetchAreaChore.StatesInstance>
{
  ...
  private static Util.IterationInstruction gatherNearbyFetchChoresVisitor(
    object obj,
    ref (ChoreConsumerState, List<Chore.Precondition.Context>, List<Chore.Precondition.Context>) context)
  {
    Unsafe.As<FetchChore>(obj).CollectChoresFromGlobalChoreProvider(context.Item1, context.Item2, (List<Chore.Precondition.Context>) null, context.Item3, true);
  ...
  {
    (ChoreConsumerState, List<Chore.Precondition.Context>, List<Chore.Precondition.Context>) context1 = (context.consumerState, succeeded_contexts, failed_contexts);
    GameScenePartitioner.Instance.VisitEntries<(ChoreConsumerState, List<Chore.Precondition.Context>, List<Chore.Precondition.Context>)>(x - radius, y - radius, radius * 2 + 1, radius * 2 + 1, GameScenePartitioner.Instance.fetchChoreLayer, new GameScenePartitioner.VisitorRef<(ChoreConsumerState, List<Chore.Precondition.Context>, List<Chore.Precondition.Context>)>(FetchAreaChore.gatherNearbyFetchChoresVisitor), ref context1);
  }
    ...
    GameStateMachine<FetchAreaChore.States, FetchAreaChore.StatesInstance, FetchAreaChore, object>.GameInstance
  {
    ...
    public void Begin(Chore.Precondition.Context context)
    {
      ...
      if (root_fetchable.MinTakeAmount)
        max_carry_weight = (float) (int) ((double) max_carry_weight / (double) root_fetchable.PrimaryElement.MassPerUnit) * root_fetchable.PrimaryElement.MassPerUnit;
      ...
      Func<object, object, Util.IterationInstruction> visitor = (Func<object, object, Util.IterationInstruction>) ((obj, _) =>
      {
      ...
      GameScenePartitioner.Instance.ReadonlyVisitEntries<object>(x2, y3, num1, num1, GameScenePartitioner.Instance.pickupablesLayer, visitor, (object) null);
      GameScenePartitioner.Instance.ReadonlyVisitEntries<object>(x2, y3, num1, num1, GameScenePartitioner.Instance.storedPickupablesLayer, visitor, (object) null);
    ...
    public void SetupDelivery()
    {
      ...
      else
      {
        ...
        if (((IEnumerable<Tag>) FetchAreaChore.StatesInstance.s_transientDeliveryTags).Contains<Tag>(delivery.chore.requiredTag))
          delivery.chore.requiredTag = Tag.Invalid;
        this.deliverables.ToArray();
        for (int index = this.deliverables.Count - 1; index >= 0; --index)
        {
          if ((UnityEngine.Object) this.deliverables[index] == (UnityEngine.Object) null || (double) this.deliverables[index].FetchTotalAmount <= 0.0 || this.deliverables[index].KPrefabID.HasTag(GameTags.MarkedForMove))
            this.deliverables.RemoveAtSwap<Pickupable>(index);
          else if (!FetchAreaChore.IsPickupableStillValidForChore(this.deliverables[index], delivery.chore))
          {
            Debug.LogWarning((object) $"Removing deliverable {this.deliverables[index]} for a delivery to {delivery.chore.destination} which did not request it");
            this.deliverables.RemoveAtSwap<Pickupable>(index);
          }
        }
        ...
        {
          this.sm.deliveryDestination.Set((KMonoBehaviour) delivery.destination, this.smi);
          this.sm.deliveryObject.Set((KMonoBehaviour) this.deliverables[0], this.smi);
          if ((UnityEngine.Object) delivery.destination != (UnityEngine.Object) null)
          {

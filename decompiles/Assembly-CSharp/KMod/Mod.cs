...
public class Mod : IHasDlcRestrictions
{
  ...
  private Mod.ArchivedVersion GetMostSuitableArchive()
  {
    ...
    Mod.ArchivedVersion archivedVersion2 = source.Where<Mod.ArchivedVersion>((Func<Mod.ArchivedVersion, bool>) (v => DlcManager.IsCorrectDlcSubscribed((IHasDlcRestrictions) v.info))).ToList<Mod.ArchivedVersion>().Where<Mod.ArchivedVersion>((Func<Mod.ArchivedVersion, bool>) (v => v.info.APIVersion == 2 || v.info.APIVersion == 0)).ToList<Mod.ArchivedVersion>().Where<Mod.ArchivedVersion>((Func<Mod.ArchivedVersion, bool>) (v => v.info.minimumSupportedBuild <= 722606)).OrderByDescending<Mod.ArchivedVersion, int>((Func<Mod.ArchivedVersion, int>) (v => v.info.minimumSupportedBuild)).FirstOrDefault<Mod.ArchivedVersion>();
  ...
  public void Install()
  {
    ...
    else
    {
      ...
      if (this.file_source == null || !this.DeleteInstallPathDirectory() || !this.CreateInstallPathDirectory() || !this.file_source.TryCopyTo(this.label.install_path))
        return;
      this.file_source = (IFileSource) new Directory(this.label.install_path);
      this.status = Mod.Status.Installed;
    }
  ...
  private bool DeleteInstallPathDirectory() => FileUtil.DeleteDirectory(this.label.install_path);

  private bool CreateInstallPathDirectory() => FileUtil.CreateDirectory(this.label.install_path);

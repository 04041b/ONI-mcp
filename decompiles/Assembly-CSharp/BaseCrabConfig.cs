...
public static class BaseCrabConfig
{
    ...
    float onDeathDropCount = 1f)
  {
    ...
    EntityTemplates.ExtendEntityToBasicCreature(false, placedEntity, anim_file, is_baby ? (string) null : "pincher_build_kanim", symbolOverridePrefix, FactionManager.FactionID.Pest, traitId, NavGridName, onDeathDropID: onDeathDropID, onDeathDropCount: onDeathDropCount, drownVulnerable: false, entombVulnerable: false, warningLowTemperature: 273.15f, warningHighTemperature: 313.15f, lethalLowTemperature: 223.15f, lethalHighTemperature: 373.15f);
    ...
    ChoreTable.Builder chore_table = builder.Add((StateMachine.BaseDef) def2, num2 != 0).Add((StateMachine.BaseDef) new CritterEmoteStates.Def(Assets.GetAnim((HashedString) "pincher_emotes_kanim"))).PopInterruptGroup().Add((StateMachine.BaseDef) new CreatureDiseaseCleaner.Def(30f)).Add((StateMachine.BaseDef) new IdleStates.Def());

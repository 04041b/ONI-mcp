  ...
  GameStateMachine<CreatureFallMonitor, CreatureFallMonitor.Instance, IStateMachineTarget, CreatureFallMonitor.Def>
{
    ...
    GameStateMachine<CreatureFallMonitor, CreatureFallMonitor.Instance, IStateMachineTarget, CreatureFallMonitor.Def>.GameInstance
  {
    ...
    {
      Vector3 pos = this.smi.transform.GetPosition() - (Vector3) this.navigator.NavGrid.GetNavTypeData(this.navigator.CurrentNavType).animControllerOffset;
      this.smi.transform.SetPosition(Grid.CellToPosCBC(Grid.PosToCell(pos), Grid.SceneLayer.Creatures) with
      {
        x = pos.x
      });

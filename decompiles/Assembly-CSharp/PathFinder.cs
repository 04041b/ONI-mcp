...
public class PathFinder
{
  ...
  public static void Initialize()
  {
    ...
    for (int index = 0; index < Grid.CellCount; ++index)
    {
      ...
      {
        HashSetPool<int, PathFinder>.PooledHashSet visited_cells = HashSetPool<int, PathFinder>.Allocate();
        GameUtil.FloodFillConditional(index, PathFinder.allowPathfindingFloodFillCb, (HashSet<int>) visited_cells);
    ...
    NavType parent_nav_type)
  {
    ...
    for (int index = 0; index < num4; ++index)
    {
      ...
      int link = pathGridCellData.link.link;
      pathGridCellData.isSubmerged = PathFinder.IsSubmerged(link);
      linksInCellRange[index] = pathGridCellData;
    }
    for (int index = 0; index < num4; ++index)
    {
      PathFinder.PotentialScratchPad.PathGridCellData pathGridCellData = linksInCellRange[index];
      ...
      int num7 = abilities.TraversePath(ref path, potential.cell, potential.navType, cost1, (int) link5.transitionId, pathGridCellData.isSubmerged) ? 1 : 0;

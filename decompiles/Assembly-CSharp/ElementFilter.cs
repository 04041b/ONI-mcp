    ...
    if (this.operational.IsOperational)
    {
      if (this.portInfo.conduitType == ConduitType.Gas || this.portInfo.conduitType == ConduitType.Liquid)
      {
        ...
        if ((double) contents1.mass > 0.0 && (double) contents2.mass <= 0.0)
        {
          ...
          if ((double) delta > 0.0)
          {
            if (this.lastelementMoved != contents1.element && contents1.element != SimHashes.Vacuum && this.portInfo.conduitType == ConduitType.Liquid)
            {
              Element elementByHash = ElementLoader.FindElementByHash(contents1.element);
              if (elementByHash != null)
                this.controller.SetSymbolTint(new KAnimHashedString("liquid"), (Color) elementByHash.substance.colour with
                {
                  a = 1f
                });
            }
            this.lastelementMoved = contents1.element;

...
public class Moppable : Workable, ISim1000ms, ISim200ms
{
  ...
  public void Sim1000ms(float dt)
  {
    if ((double) this.amountMopped <= 0.0)
      return;
    PopFX popFx = PopFXManager.Instance.SpawnFX(this.lastElementMopped == null ? PopFXManager.Instance.sprite_Resource : Def.GetUISprite((object) this.lastElementMopped).first, (Sprite) null, GameUtil.GetFormattedMass(-this.amountMopped), this.transform, Vector3.zero);
    if ((UnityEngine.Object) popFx != (UnityEngine.Object) null && this.lastElementMopped != null && this.lastElementMopped.substance != null)
      popFx.SetIconTint((Color) this.lastElementMopped.substance.colour);
    this.amountMopped = 0.0f;
    this.lastElementMopped = (Element) null;
  }
  ...
  private void OnCellMopped(Sim.MassConsumedCallback mass_cb_info, object data)
  {
    ...
    SubstanceChunk chunk = LiquidSourceManager.Instance.CreateChunk(element, mass_cb_info.mass, mass_cb_info.temperature, mass_cb_info.diseaseIdx, mass_cb_info.diseaseCount, Grid.CellToPosCCC(cell, Grid.SceneLayer.Ore));

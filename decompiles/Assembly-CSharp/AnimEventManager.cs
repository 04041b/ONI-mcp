...
public class AnimEventManager : Singleton<AnimEventManager>
{
  ...
  {
    AnimEventManager.IndirectionData data1 = this.indirectionData.GetData(handle);
    KCompactedVector<AnimEventManager.AnimData> kcompactedVector1 = data1.isUIData ? this.uiAnimData : this.animData;
    AnimEventManager.AnimData data2 = kcompactedVector1.GetData(data1.eventDataHandle) with
    {
      frameRate = anim.frameRate,
      totalTime = anim.totalTime,
      numFrames = anim.numFrames
    };
    kcompactedVector1.SetData(data1.animDataHandle, data2);
    KCompactedVector<AnimEventManager.EventPlayerData> kcompactedVector2 = data1.isUIData ? this.uiEventData : this.eventData;
    AnimEventManager.EventPlayerData data3 = kcompactedVector2.GetData(data1.eventDataHandle) with
    {
      events = GameAudioSheets.Get().GetEvents(anim.id)
    };
    if (data3.events == null)
      data3.events = AnimEventManager.emptyEventList;
    kcompactedVector2.SetData(data1.eventDataHandle, data3);
  }

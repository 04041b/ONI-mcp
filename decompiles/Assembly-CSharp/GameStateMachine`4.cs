  ...
  where MasterType : IStateMachineTarget
{
  ...
  protected static StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.Parameter<int>.Callback IsLTEZero_int = (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.Parameter<int>.Callback) ((smi, p) => (double) p <= 0.0);
    ...
    StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.Transition
  {
    ...
    private System.Action<object, object> callbackDispatcher;
      ...
      : base(name, (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State) source_state, (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State) target_state, idx, (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.Transition.ConditionCallback) null)
    {
      ...
      this.callbackDispatcher = (System.Action<object, object>) ((context, data) => this.OnCallback(Unsafe.As<StateMachineInstanceType>(context)));
    }
    ...
    {
      StateMachineInstanceType machineInstanceType = smi as StateMachineInstanceType;
      Debug.Assert((object) machineInstanceType != null);
      return base.Register((StateMachine.Instance) machineInstanceType) with
      {
        handlerId = this.target.Get(machineInstanceType).Subscribe(-1582839653, this.callbackDispatcher, (object) machineInstanceType)
      };
    }
    ...
    StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.Transition
  {
    ...
    public override StateMachine.BaseTransition.Context Register(StateMachine.Instance smi)
    {
      StateMachineInstanceType machineInstanceType = smi as StateMachineInstanceType;
      Debug.Assert((object) machineInstanceType != null);
      StateMachine.BaseTransition.Context context = base.Register((StateMachine.Instance) machineInstanceType);
      GameObject gameObject;
      if (this.globalEventSystemCallback != null)
      {
        gameObject = this.globalEventSystemCallback(machineInstanceType).gameObject;
      }
      else
      {
        gameObject = this.target.Get(machineInstanceType);
        if ((UnityEngine.Object) gameObject == (UnityEngine.Object) null)
          throw new InvalidOperationException($"TargetParameter: {this.target.name} is null");
      }
      context.handlerId = gameObject.Subscribe((int) this.evtId, this.callbackDispatcher, (object) machineInstanceType);
      return context;
    }
    ...
    StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State
  {
    ...
    public GameStateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State ToggleCritterEmotion(
      CritterEmotion emotion,
      Func<StateMachineInstanceType, bool> condition_callback = null)
    {
      StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.TargetParameter state_target = this.GetStateTarget();
      this.Enter($"AddCritterEmotion({emotion.id})", (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State.Callback) (smi =>
      {
        if (condition_callback != null && !condition_callback(smi))
          return;
      ...
      if (condition_callback != null)
        this.Update($"AddCritterEmotion({emotion.id})", (System.Action<StateMachineInstanceType, float>) ((smi, dt) =>
        {
          CritterEmoteMonitor.Instance smi1 = state_target.Get(smi).GetSMI<CritterEmoteMonitor.Instance>();
          if (condition_callback(smi))
            smi1?.AddCritterEmotion(emotion);
          else
            smi1?.RemoveCritterEmotion(emotion);
        }));
      this.Exit($"RemoveCritterEmotion({emotion.id})", (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State.Callback) (smi => state_target.Get(smi).GetSMI<CritterEmoteMonitor.Instance>()?.RemoveCritterEmotion(emotion)));
      return this;
    }
    ...
    public GameStateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State OnSignal(
      ...
      GameStateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State state,
      StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.Parameter<StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.SignalParameter>.Callback callback)
    {
      this.ParamTransition<StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.SignalParameter>((StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.Parameter<StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.SignalParameter>) signal, state, callback);
      return this;
    }
      ...
      HashedString anim)
    {
      ...
      this.Enter($"PlayAnim({anim.ToString()}, {mode.ToString()})", (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State.Callback) (smi =>
      {
    ...
    public GameStateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State PlayAnim(
      Func<StateMachineInstanceType, HashedString> anim_cb,
      KAnim.PlayMode mode = KAnim.PlayMode.Once)
    {
      StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.TargetParameter state_target = this.GetStateTarget();
  ...
  public class GameEvent : StateEvent
  {
      ...
      : base(id.ToString())
    {
      ...
      this.callbackDispatcher = new System.Action<object, object>(this.InvokeCallback);
      this.globalEventSystemCallback = global_event_system_callback;
    }
    ...
    public override StateEvent.Context Subscribe(StateMachine.Instance smi)
    {
      ...
      StateMachineInstanceType machineInstanceType = (StateMachineInstanceType) smi;
      if (this.globalEventSystemCallback != null)
      {
        KMonoBehaviour kmonoBehaviour = this.globalEventSystemCallback(machineInstanceType);
        context.data = kmonoBehaviour.Subscribe((int) this.id, this.callbackDispatcher, (object) machineInstanceType);
      }
      else
        context.data = this.target.Get(machineInstanceType).Subscribe((int) this.id, this.callbackDispatcher, (object) machineInstanceType);
      return context;
    }

  ...
  FlowUtilityNetwork.IItem
{
  ...
  {
    int cell = ((Boxed<bool>) data).value ? Grid.PosToCell(this.transform.GetPosition()) : -1;
    this.GetFlowVisualizer().SetHighlightedCell(cell);
  }

  private void OnConduitFrozen(object data)
  {
    this.BoxingTrigger<BuildingHP.DamageSourceInfo>(-794517298, new BuildingHP.DamageSourceInfo()
    {
      damage = 1,

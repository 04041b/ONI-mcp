...
public class SolidConduitFlow : IConduitFlow
{
  ...
  private void TryAdd(List<SolidConduitFlow.Conduit> new_path)
  {
    ...
    {
      List<SolidConduitFlow.Conduit> path = this.pathList[index5];
      if (new_path.Count >= path.Count)
      {
        bool flag = false;
        int idx1 = path[0].idx;
        int idx2 = path[path.Count - 1].idx;
        int num1 = -1;
        int num2 = -1;
        for (int index6 = 0; index6 < new_path.Count; ++index6)
        {
          if (new_path[index6].idx == idx1)
          {
            num1 = index6;
            break;
          }
        }
        for (int index7 = 0; index7 < new_path.Count; ++index7)
        {
          if (new_path[index7].idx == idx2)
          {
            num2 = index7;
            break;
          }
        ...
        if (num1 != -1 && num2 != -1)
        {
          ...
          while (index8 < num2)
          {
            if (new_path[index8].idx != path[index9].idx)
            {
              flag = false;
              break;
            }
  ...
  public void SetContents(int cell, Pickupable pickupable)
  {
    ...
    if ((UnityEngine.Object) pickupable != (UnityEngine.Object) null)
    {
      ...
      pickupable.Trigger(856640610, (object) BoxedBools.True);
    }
    ...
    if (conduit.idx != -1)
    {
      ...
      if (conduitContents.pickupableHandle.IsValid())
      {
        ...
        if ((bool) (UnityEngine.Object) pickupable)
          pickupable.Trigger(856640610, (object) BoxedBools.False);

...
public class CodexScreen : KScreen
{
    ...
    CodexScreen.HistoryDirection historyMovement = CodexScreen.HistoryDirection.NewArticle)
  {
    ...
    if (!CodexCache.entries.ContainsKey(id))
    {
      ...
      {
        Debug.LogWarning((object) ("Codex SubEntry not found for target id: " + id));
        id = "PAGENOTFOUND";
      }
    }
    if (CodexCache.entries[id].disabled)
    {
      Debug.LogWarning((object) ("Codex Entry disabled for target id: " + id));
      id = "PAGENOTFOUND";
    }
    if (string.IsNullOrEmpty(articleName))
      articleName = STRINGS.UI.StripLinkFormatting(CodexCache.entries[id].name);
    ICodexWidget codexWidget = (ICodexWidget) null;
    ...
    while (this.contentContainers.transform.childCount > 0)
    {
      ...
      {
        if (DlcManager.IsCorrectDlcSubscribed(CodexCache.entries[this.activeEntryID].contentContainers[index1].content[index2] as IHasDlcRestrictions))
        {
    ...
    for (int index3 = 0; index3 < CodexCache.entries[id].contentContainers.Count; ++index3)
    {
      ...
      if (Game.IsCorrectDlcActiveForCurrentSave((IHasDlcRestrictions) contentContainer))
      {
        if (!string.IsNullOrEmpty(contentContainer.lockID) && !Game.Instance.unlocks.IsUnlocked(contentContainer.lockID))
        {
        ...
        else
        {
          ...
          {
            foreach (ICodexWidget restrictions in contentContainer.content)
            {
              if (Game.IsCorrectDlcActiveForCurrentSave(restrictions as IHasDlcRestrictions))
              {
                GameObject gameObject6 = this.ContentUIPools[restrictions.GetType()].GetFreeElement(gameObject5, true).gameObject;
                restrictions.Configure(gameObject6, this.displayPane, this.textStyles);
                if (restrictions == codexWidget)
